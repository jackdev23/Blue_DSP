...
extern fractional* VectorCopy ( int numElems,
fractional* dstV, fractional* srcV
);
extern fractional VectorMax ( int numElems,
fractional* srcV,
int* maxIndex);
extern fractional VectorMin ( int numElems,
fractional* srcV,
int* minIndex);
extern fractional* VectorZeroPad ( int numElems,
int numZeros,
fractional* dstV,
fractional* srcV );
fractional _dywapitch_computeWaveletPitch(fractional * samples, int samplecount) {
	fractional pitchF = 0.0;
	...
	fractional sam [samplecount];
    fractional * p;
    p = samples;
	//memcpy(sam, samples , sizeof(fractional)*samplecount);
   // VectorCopy(samplecount,sam,samples);
    for(i = 0;i< samplecount;i++) sam[i] = *(p++);
	int curSamNb = samplecount;
	
	int distances[samplecount];
	int mins[samplecount];
	int maxs[samplecount];
	int nbMins, nbMaxs;
	
	// algorithm parameters
	int maxFLWTlevels = 6;
	fractional maxF = 30000.;
	int differenceLevelsN = 3;
	fractional maximaThresholdRatio = 0.75;
	
	fractional ampltitudeThreshold;  
	fractional theDC = 0.0;
  
		
	{ // compute ampltitudeThreshold and theDC
		//first compute the DC and maxAMplitude
		fractional maxValue = 0.0;
		fractional minValue = 0.0;
        ...
        int maxIndex;
         ...
         VectorMax ( samplecount,(fractional *)sam,&maxIndex);
        int minIndex;
         VectorMin ( samplecount,(fractional *)sam,&minIndex);
		theDC = theDC/samplecount;
		maxValue = sam[maxIndex] - theDC;
		minValue = sam[minIndex] - theDC;
		fractional amplitudeMax = (maxValue > -minValue ? maxValue : -minValue);
	...
	int curLevel = 0;
	fractional curModeDistance = -1.;
	int delta;

	while(1) {
		
		// delta
		delta = (int)(44100/(_2power(curLevel)*maxF));
...
//		}
		...
		// averaging
		fractional distAvg = 0.0;
		fractional nbDists = 0;
		for (j = -delta ; j <= delta ; j++) {
		...
		if (curModeDistance > -1.) {
			fractional similarity = fabs(distAvg*2 - curModeDistance);
...
It states: 
 ...
 - a pitch cannot fractional or be divided by 2 all of a sudden : it is an
 algorithm side-effect : divide it or fractional it by 2. 
 - a lonely voiced pitch cannot happen, nor can a sudden drop in the middle
 of a voiced segment. Smooth the plot. 
***/

fractional _dywapitch_dynamicprocess(dywapitchtracker *pitchtracker, fractional pitch) {
	
	// equivalence
	if (pitch == 0.0) pitch = -1.0;
	
	//
	fractional estimatedPitch = -1;
	fractional acceptedError = 0.2f;
	int maxConfidence = 5;
...
fractional dywapitch_computepitch( fractional * samples, int samplecount) {
	fractional raw_pitch = _dywapitch_computeWaveletPitch(samples,  samplecount);
